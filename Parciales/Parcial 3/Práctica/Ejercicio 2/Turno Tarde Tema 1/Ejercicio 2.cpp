...
2) Generar una lista enlazada con punteros con los números de tarjetas y nombre de cliente cuyo cupo es superior a $400.000
3) Ingresar un número de tarjeta y eliminarlo de la lista generada.
...
void elimina(puntero &xp){                  //esta función sirve para eliminar un nodo de una lista.
	...
	printf("\n Ingrese el código de una tarjeta para eliminarlo: ");
	scanf("%d", &codigo);
    if(xp->codi == codigo){
        p = xp;
        xp = xp->sig;
        ...
        if(p != NULL){
            ...
            free(p);
            printf("\n Tarjeta eliminada con éxito. \n");
        } else printf("\n La Tarjeta no se encontró en la lista \n");
    }
}
